...
# Learnable laws are laws that can be trained using a regressor (e.g., a linear regression function, a neural network, etc).
# They are used to map input variables to a target variable inside the iceflow model.
...
# The output of the law definition above states that it maps the long term air temperature `T` to a float value which corresponds to the Glen coefficient `A`.
# It is defined as a neural network that takes as input `T` and returns `A`.
...
# As explained in the [Sensitivity analysis](./sensitivity.md) section, ODINN computes vector-Jacobian products (VJPs) using AD in order to evaluate the gradient of the loss function.
...
# For this specific law, the VJPs are efficiently implemented and the user does not have to worry about this.
...
# The output above after `A_law = LawA(nn_model, params)` shows that the law is applied only once at the beginning of the simulation.
# Additionally, it inform us that custom VJPs used to compute the gradient are precomputed
# (that is, VJPs are computed before solving the adjoint iceflow PDE, refer to the [laws VJP customization](./vjp_laws.md) tutorial for more information).
