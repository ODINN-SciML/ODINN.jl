...
# And finally, we just run the simulation. This will run the adjoint method to compute the gradients and then use the ADAM and LBFGS optimizers
...
plot_law(prediction.model.iceflow.A, functional_inversion, law_input, nothing)
