...
if GROUP == "All" || GROUP == "Core3"
    @testset "Adjoints tests of SIA equation with A as target" begin
        ...
        @testset "Manual implementation of the discrete adjoint with discrete VJP vs finite differences (initial condition)" test_grad_finite_diff(DiscreteAdjoint(VJP_method = DiscreteVJP()); thres = [2e-2, 8e-5, 2e-2], train_initial_conditions = true)
