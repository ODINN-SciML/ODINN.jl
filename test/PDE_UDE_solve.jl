...
function pde_solve_test(atol)
    ...
    PDE_preds = @time generate_ref_dataset(gdirs_climate, tspan)

    # Run one epoch of the UDE training
    θ = zeros(10) # dummy data for the NN
    UA_f = zeros(10)
    UDE_settings, train_settings = ODINN.get_default_training_settings!(gdirs_climate)
    context_batches = ODINN.get_UDE_context(gdirs_climate, tspan)
    H_V_preds = @time ODINN.predict_iceflow(θ, UA_f, gdirs_climate_batches, context_batches, UDE_settings; testmode=true) # Array{(H_pred, V̄x_pred, V̄y_pred, rgi_id)}
    ...
    for PDE_pred in PDE_preds
        ...
        ODINN.plot_test_error(UDE_pred, test_ref, "Vx", rgi_id, atol)
        ODINN.plot_test_error(UDE_pred, test_ref, "Vy", rgi_id, atol)
        ...
        @test isapprox(UDE_pred[2], test_ref["Vx"], atol=atol)
        @test isapprox(UDE_pred[3], test_ref["Vy"], atol=atol)
        end
    end
    end
end
