...
```julia
# We first need to declare the type for the input variable, with any fields that
# might be needed
...
# And then, using multiple dispatch, we specify the righ `get_input` function for
# this type, i.e. how to get it
function get_input(cpdd::InpCPDD, simulation, glacier_idx, t)
    window = cpdd.window
    glacier = simulation.glaciers[glacier_idx]
    # We trim only the time period between `t` and `t - x`, where `x` is the PDD time
    # window defined in the physical parameters.
