...
function plot_glacier_dataset(gdirs_climate, PDE_refs, display=false)
    if plots
        ...
        for (i, ax) in enumerate(axsS)
            ...
            name = gdirs[i].name
            ax.title = "$name - $rgi_id"
            S = PDE_refs["S_refs"][i] # final surface elevation

            H = glacier_gd.consensus_ice_thickness.data # initial ice thickness conditions for forward model
            B = PDE_refs["B_refs"][i] # glacier bedrock
            S₀ = B .+ H
            S_diff = S .- S₀

            push!(Ss, fillNaN(S_diff))
            push!(hm_Ss, Makie.heatmap!(ax, S_diff, colormap=:balance))
        end
        minS = minimum(minimum.(Ss))
        maxS = maximum(maximum.(Ss)) 
        Colorbar(figS[2:3,m+1], limits=(minH,maxH), colormap=:balance, label="Surface altitude difference (m)")
