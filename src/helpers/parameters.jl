...
const ρ = Ref{Float64}(900.0f0)                 # Ice density [kg / m^3]
const g = Ref{Float64}(9.81f0)                  # Gravitational acceleration [m / s^2]
const n = Ref{Float64}(3.0f0)                   # Glen's flow law exponent
const ϵ = Ref{Float64}(1e-3)                    # small number
const maxA = Ref{Float64}(8e-17)
const minA = Ref{Float64}(8.5e-20)
const maxT = Ref{Float64}(1.0f0)
const minT = Ref{Float64}(-25.0f0)
...
const ref_max_MB = Ref{Float64}(5.0f0)
const ref_min_MB = Ref{Float64}(-10.0f0)
use_MB = true
