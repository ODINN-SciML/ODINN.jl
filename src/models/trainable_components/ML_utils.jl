...
function build_default_NN(; n_input = 1, lightNN = false)
    if lightNN
        ...
        architecture = Lux.Chain( # Light network for debugging
            ...
            Dense(3, 1, sigmoid)            # Dense(n_input, 3, x -> sigmoid.(x); init_weight = Lux.glorot_normal),            # Dense(3, 1, sigmoid; init_weight = Lux.glorot_normal)
        )
    else
...
function build_simulation_batch(
    ...
    massbalance = get_mb_model(simulation.model.mass_balance, i)
...
# Notes
  ...
  - The function currently defaults to `nbatches = 1`, meaning no actual batching is performed. This can be updated to use `simulation.parameters.hyper.batchsize` for dynamic batching.    # TODO: we need to change how this is done here, with a manual =1 batch size
  - If the simulation results are empty, the function creates batches with empty results objects. #simulation.parameters.hyper.batchsize
"""
...
function update_training_state!(simulation::S, l) where {S <: Simulation}
    ...
    if simulation.parameters.hyper.current_minibatch >= length(simulation.glaciers)
        ...
        println("Epoch #$(simulation.parameters.hyper.current_epoch): ",
            simulation.parameters.hyper.loss_epoch)
