...
function run()
    ...
    gdirs = ODINN.init_gdirs(rgi_ids)
    ...
    gdirs_climate = ODINN.get_gdirs_with_climate(gdirs, true)
    ...
    if create_ref_dataset 
        ...
        H_refs, V̄x_refs, V̄y_refs = @time ODINN.generate_ref_dataset(gdirs_climate)
    ...
    ODINN.plot_glacier_dataset(gdirs_climate, PDE_refs)
    ...
    if retrain
        ...
        iceflow_trained, UA = @time ODINN.train_iceflow_UDE(gdirs_climate, train_settings, PDE_refs, θ_trained) # retrain
        ...
        iceflow_trained, UA = @time ODINN.train_iceflow_UDE(gdirs_climate, train_settings, PDE_refs)
        θ_trained = iceflow_trained.minimizer
        println("Saving NN weights...")
        jldsave(joinpath(ODINN.root_dir, "data/trained_weights.jld2"); θ_trained, current_epoch)
