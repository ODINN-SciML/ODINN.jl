...
```julia
result = eval_law(simulation.model.iceflow.A, simulation, glacier_idx, (; T = 273.15), θ)    # Initialize the cache to be able to make an inference of the law
```
"""
function eval_law(law::AbstractLaw, simulation::Simulation,
...
function T_A_Alaw(simulation::Simulation, glacier_idx::Integer, θ, t::AbstractFloat)
    _inputs_A_law = (; T = iAvgScalarTemp())
    ...
    T = get_input(iAvgScalarTemp(), simulation, glacier_idx, t)
