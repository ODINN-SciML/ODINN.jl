...
function LawU(
    ...
    prescale = !isnothing(prescale_bounds) ? X -> _ml_model_prescale(X, prescale_bounds) :
               identity
    ...
    init_cache_interp = function (simulation, glacier_idx, θ)
        ...
        grads = [zero(θvec) for i in 1:length(H_nodes), j in 1:length(∇S_nodes)]
    ...
    p_VJP! = let smodel = smodel, prescale = prescale, postscale = postscale
        function (cache, vjpsPrepLaw, inputs, θ)
            ...
            grads = [zeros(only(size(θ)))
                     for i in 1:length(nodes_H), j in 1:length(nodes_∇S)]
...
function LawA(
    ...
    callback_freq = if isa(params.UDE.grad, SciMLSensitivityAdjoint) ||
                       isa(params.UDE.grad, DummyAdjoint) ||
