...
plot_epoch = Plots.scatter!(Temps, As_pred, label = "Predicted A",
    xlabel = "Long-term air temperature (°C)", yticks = [
        0.0, 1e-17, 1e-18, params.physical.maxA],
...
Plots.savefig(plot_initial_condition,
    "scripts/MWEs/inversion_A/figures/MWE_inversion_A_initial_condition_ref.pdf")

plot_initial_condition = Plots.heatmap(H₀_pred[1], colormap = :Blues, clim = (0.0, 200.0));
Plots.savefig(plot_initial_condition,
    "scripts/MWEs/inversion_A/figures/MWE_inversion_A_initial_condition_pred.pdf")

plot_initial_condition = Plots.heatmap(H₀_ref[1] .- H₀_pred[1], colormap = :bwr, clim = (
    -10.0, 10.0));
Plots.savefig(plot_initial_condition,
    "scripts/MWEs/inversion_A/figures/MWE_inversion_A_initial_condition_diff.pdf")
