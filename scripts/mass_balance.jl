...
function train()
    ...
    global temperature_toy = 100.0f0*sin.((1:500) ./ 20)
    ...
    l3 = @layout [a b]
    p3₁ = plot(X_raw, Ŷp_raw', lw = 3, xlabel = "Snowfall (P)",
        ylabel = "Accumulation (α(P))", label = "Up (inferred function)")
    plot!(X_raw, Yp_raw', l2 = 3, lw = 3, label = "α (true function")
    p3₂ = plot(X_raw, Ŷt_raw', lw = 3, xlabel = "Temperature (T)",
        ylabel = "Ablation (μ(T))", label = "Ut (inferred function)")
...
function sindy()
    ...
    syn_range = 1:100
    Ψ_p_range = SINDy(Float64.(syn_range), Float64.(Up(syn_range')), basis_p, opt,
        p = [0], maxiter = 20000, denoise = true, normalize = true)
    println(Ψ_p_range)
    ...
    Ψ_p = SINDy(Xp, Float64.(Ŷ_acc), basis_p, opt, p = [0], maxiter = 200000,
        denoise = false, normalize = true, convergence_error = 1e-8)
    println(Ψ_p)
    print_equations(Ψ_p)
    p̂_p = parameters(Ψ_p)
    ...
    println("SINDy on synthetic range predicted data \n")
    syn_range = 1:100
    Ψ_t_range = SINDy(Float64.(syn_range), Float64.(Ut(syn_range')), basis_p, opt,
        p = [0], maxiter = 20000, denoise = true, normalize = true)
    println(Ψ_t_range)
    ...
    println("SINDy on predicted data \n")
    Ψ_t = SINDy(Xt, Float64.(Ŷ_abl), basis_p, opt, p = [0], maxiter = 200000,
        denoise = false, normalize = true, convergence_error = 1e-8)
    println(Ψ_t)
    print_equations(Ψ_t)
    p̂_t = parameters(Ψ_t)
