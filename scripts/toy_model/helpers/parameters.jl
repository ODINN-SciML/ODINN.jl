...
const maxT = 1
const minT = -25
const minA_out = 8.5e-3 # /!\     # these depend on predict_A̅, so careful when changing them!
const maxA_out = 8
...
(@isdefined A_values) || (const A_values = hcat(A_values_sec[1,:], A_values_sec[2,:].*60*60*24*365.25)')
